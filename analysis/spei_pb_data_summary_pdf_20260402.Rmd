---
title: "Spectacled Eiders are still being exposed to lead on the Yukon-Kuskokwim Delta, Alaska (DRAFT 20260406)"
author: "Daniel Rizzolo, U.S. Fish and Wildlife Service, daniel_rizzolo@fws.gov"
...
```{r load_wrangle_fws_data, warning = FALSE, message = FALSE, echo = FALSE}
...
```{r tbl_n, warning = FALSE, message = FALSE, echo = FALSE}
...
```{r plot_validation, fig.width = 4, fig.height = 4, fig.cap = "Fig. 1 Lead concentration in Spectacled Eider blood samples analyzed using both mass spectrometry and voltammetry (n = 23). The red line is the mean association between the two methods estimated with a linear model (log slope = 1.05, 95% CI: 0.86, 1.23). A slope > 1 indicates that voltammetry tends to underestimate lead concentrations relative to mass spectrometry.", warning = FALSE, message = FALSE, echo = FALSE}
...
p.m67 <- ggplot(data = pb_limits_wide, aes(y = log(pb_blood_ppm.mass_spec), x = log(pb_blood_ppm.voltammetry)))+
  ...
  xlab("log(lead) from voltammetry") +
  ylab("log(lead) from mass spec") +
  theme_bw()
p.m67
```
```{r adjust_estimates, warning = FALSE, message = FALSE, echo = FALSE  }
...
p.all_exp_tut <- ggplot(plot_data_all_tut, aes(fill = Period, y = mean_exp, x = reorder(x = period_sampling, order))) +
  ...
  theme_bw()
```
```{r panel_plot, fig.width = 7, fig.height = 9, fig.cap = "Fig. 2 Lead exposure probabilities for Spectacled Eiders based on blood lead concentration values \u2265 0.20 ppm by study site and sampling period. Error bars are 95% confidence intervals. Numbers above bars are estimated probabilities of exposure. Sample sizes are presented at the base of each bar. Estimates indicate exposure was still occurring during the 2018-2025 sampling period, sometimes at rates similar to those from the 1990s.", warning = FALSE, message = FALSE, echo = FALSE}
...
```{r render, warning = FALSE, echo = FALSE}
# code to render: copy/paste/run the code below in the R console to get PDF output
#rmarkdown::render(
#    input = "analysis/spei_pb_data_summary_pdf_20260402.Rmd",
#    output_format = "pdf_document",
#    output_file = "../docs/report/spei_pb_data_summary_20260402.pdf",
#    envir = new.env(),                 # clean environment
#    knit_root_dir = "C:/Users/drizzolo/OneDrive - DOI/Desktop/Documents/projects/spei_lead"   
#)
```
