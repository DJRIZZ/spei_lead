...
m <- leaflet() %>%
  ...
  addCircles(data = eider_lead_fws,
             ~ lon_jit,
             ~ lat_jit,
             color = ifelse(eider_lead_fws$pb_ppm < 0.20, "blue","red"),
             radius = ~ pb_ppm * 100,
             #group ="FourWeeks",
...
lead_spei$exposed <- ifelse(lead_spei$pb_ppm >= 0.20, 1,0)
lead_spei$notexp <- ifelse(lead_spei$pb_ppm <= 0.20, 1,0)
...
for(var in unique(lead_spei$site_stage)){
  print(ggplot(lead_spei[lead_spei$site_stage==var,], aes(pb_ppm)) +
