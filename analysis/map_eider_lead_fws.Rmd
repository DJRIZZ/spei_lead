...
## Plotted data
Capture locations of eiders (mostly Spectacled Eiders, but also 2 King Eiders on the ACP and 1 Common Eider at Kigigak) tested for blood lead levels. Blue symbols are individuals < 0.2 ppm lead. Red symbols are >= 0.20 ppm and scaled to concentration. Hover your cursor over points to get associated info. 

```{r plot, warning = FALSE}
eider_lead_fws <- read.csv("data/eider_lead_fws.csv", header = TRUE)

# jitter lat for captures at same capture site
lat_jit <- ifelse(diff(eider_lead_fws$lat) == 0, eider_lead_fws$lat + 0.001, eider_lead_fws$lat)
lat_jit[206] <-eider_lead_fws[206, 8]
eider_lead_fws$lat_jit <- lat_jit

lon_jit <- ifelse(diff(eider_lead_fws$lon) == 0, eider_lead_fws$lon + 0.001, eider_lead_fws$lon)
lon_jit[206] <-eider_lead_fws[206, 9]
...
t1<-kbl(lead_table[ ,1:5], caption="Spectacled Eider blood lead levels above 0.20 ppm (exposed) and below 0.20 ppm (not exposed) from samples collected on the Yukon-Kuskokwim Delta and Arctic Coastal Plain, Alaska, 2018-2022.")
