...
```{r load_wrangle_fws_data, echo = FALSE}
...
```{r fig_validation, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 6, fig.width = 6, fig.cap = "<div style = 'text-align:left'>Fig. 1 Lead concentration in Spectacled Eider blood samples analyzed using both mass spectrometry and voltammetry (n = 23). The red line is the mean association between the two methods estimated with a linear model (slope = 1.05, 95% CI: 0.86, 1.23). A slope > 1 indicates that voltammetry tends to underestimate lead concentrations relative to mass spectrometry.</div>"}

# plot mass spec vs. voltammetry values with hypothesized slope = 1 line and actual slope of the association from the m67 linear model
p.m67 <- ggplot(data = pb_limits_wide, aes(y = log(pb_blood_ppm.mass_spec), x = log(pb_blood_ppm.voltammetry)))+
  ...
  xlab("log blood lead - voltammetry") +
  ylab("log blood lead - mass spec") +
...
p.all_exp_tut <- ggplot(plot_data_all_tut, aes(fill = Period, y = mean_exp, x = reorder(x = period_sampling, order))) +
  ...
  theme_bw()
```
```{r panel_plot, waring = FALSE, ech = FALSE, fig.width=7, fig.height=9, fig.cap = "<div style = 'text-align:left'>Fig. 2 Lead exposure probabilities for Spectacled Eiders based on blood lead concentration values \u2265 0.20 ppm by study site and sampling period. Error bars are 95% confidence intervals. Numbers above bars are estimated probabilities of exposure. Sample sizes are presented at the base of each bar. Estimates indicate exposure was still occurring during the 2018-2025 sampling period, sometimes at rates similar to those from the 1990s. </div>"}
...
#rmarkdown::render(
#    input = "analysis/spei_pb_data_summary_docx_202600402.Rmd",
#    output_format = "word_document",
#    output_file = "spei_pb_data_summary_docx_202600402.docx",
